...
--
-- Returns per day:
--   trials, conversions (# who became active/paying), avg_active, median_active, max_active,
...
drop function if exists public.trial_daily_activity(date, date);

create or replace function public.trial_daily_activity(start_date date default null, end_date date default null)
returns table(d date, trials int, conversions int, avg_active numeric, median_active numeric, max_active int, partial boolean, hist int[])
language sql stable as $$
  with t as (
    select user_id, (trial_started_at at time zone 'UTC')::date as d, trial_started_at,
      (became_active_at is not null) as converted
    from user_info
  ...
  a as (
    select t.d, t.user_id, t.converted,
    ...
    group by t.d, t.user_id, t.converted
  ),
  hist as (
  ...
  select s.d, s.trials, s.conversions, s.avg_active, s.median_active, s.max_active, s.partial, h.hist
